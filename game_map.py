...
class MapSquare:
    ...
    def get_desc(self):
        """Returns description of class object
    ...
    def is_found(self) -> bool:
        """Reveals hidden object (if present)
    ...
    def get_object_data(self) -> dict:
        """Used to get MapSquare class object information for save file

            Returns:
                dict: object information in save file formatting
        """
        return {"nr": None, "category": self.categ, "level": "0", "name": self.name, "desc": self.desc,
                "is_hidden": str(int(self.is_hidden)), "Xval": str(self.coord[0]),
                "Yval": str(self.coord[1]), "loot": "0"}


class Location(MapSquare):
    def __init__(self, categ:str, name: str, desc: str, is_hidden: bool, Xval: int, Yval: int, loot: str):
        """Location class object that is child of the MapSquare class. Used to define location on map

        Args:
            categ (str): category of object
            name (str): name of object
            desc (int): description of object
            is_hidden (bool): is given object hidden
            Xval (int): coordinates of object on x-axis
            Yval (int): coordinates of object on y-axis
    ...
    def pickup_loot(self):
        """Removes loot from object if it was picked up
        """
        self.loot = "0"

    def get_object_data(self) -> dict:
        """Used to get Location class object information for save file

            Returns:
                dict: object information in save file formatting
            """
        object_data = super().get_object_data()
        object_data['loot'] = self.loot
        return object_data
...
class MapArea:
    ...
    def current_map_locations(self):
        """Creates list of current map locations, used to save the game progress
        ...
        for col in self.game_map:  # for every column in map matrix
            for row in col:  # for every row in current column
                if row is not None:
                    current_loc = row.get_object_data()
                    current_loc['nr'] = location_count
    ...
    def get_location_by_coords(x: int, y: int, locations):
        """Returns object that is present at given coordinates
    ...
    def generate_locations(items):
        """Used to create class objects of map locations
        ...
        try:
            for item in items:
                if str(item[1]) == 'location':  # item is location class object
                    new_location = Location("location", item[3], item[4], bool(int(item[5])), int(item[6]),
                                            int(item[7]), item[8])
